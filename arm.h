...
public:
    ...
    void moveToPos(float _target_x,float _target_y,float _target_z,float _use_t_long);
     ...
     ArmAng getAng();
     //单位  默认度
     float get_c1(bool _ang=true){if(_ang) return c1*180/pi;else return c1;};
     float get_c2(bool _ang=true){if(_ang) return c2*180/pi;else return c2;};
     float get_c3(bool _ang=true){if(_ang) return c3*180/pi;else return c3;};
